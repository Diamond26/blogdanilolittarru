...
router.post('/:postId', optionalAuth, async (req, res) => {
  ...
  try {
    const [postRows] = await db.execute('SELECT id FROM posts WHERE id = ? AND status LIKE ? LIMIT 1', [postId, statusFilter]);
